...
## Роли (RBAC, SRS §3)
Гость → Родитель/студент → Учреждение → Модератор → Администратор МОН (MVP/Core). Соискатель — вторая волна.

## Ключевые сущности (SRS §7)
`User`, `Institution` (вкл. поля развозки/питания), `Rating` (9 метрик), `Review`, `Teacher`, `Message`. Точная схема `Institution` — см. SRS §7.2, не выдумывать поля мимо неё.
