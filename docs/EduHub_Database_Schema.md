...
Каждая таблица — в своей схеме, пишет в неё только сервис-владелец схемы (`.claude/rules/architecture.md`). Кросс-схемные FK запрещены, кроме двух явно согласованных исключений (см. ниже). Все ID — `UUID`, всё время — `TIMESTAMPTZ`. Двуязычные поля, которые платформа или учреждение готовит заранее (название, описание, программа) — `JSONB {ru, tg}` (симметрично `Bi` из `web/lib/data.ts:23`); текст, который пишет живой человек в моменте (отзыв, сообщение в чате, обращение) — обычный `TEXT`, на любом языке, независимо от языка интерфейса. Гео — `GEOGRAPHY(Point,4326)` с `GIST`-индексом. Опциональные поля отмечены `NULL`; на Go-стороне им соответствует `*T`, не нулевое значение (`.claude/rules/go.md`).
...
- `auth.employment_claims.institution_id → catalog.institutions(id)` — тот же паттерн, для верификации трудоустройства (см. схему `auth`).
...
Владелец: сервис Auth (веха 2). Аккаунты, сессии, минимальная привязка ребёнка для верификации отзывов, верификация трудоустройства.

### `auth.users`

Один аккаунт на человека (без ветки «кто вы» при регистрации — учреждение регистрирует не юрлицо, а человек-представитель). Основные каналы входа — email/пароль и Google, равноправно; телефон — опциональное поле второй очереди (SMS-верификация в РТ — платная услуга внешнего провайдера, не оправдана на холодном старте, в отличие от email, не требующего внешнего платного канала).

| Поле | Тип | Nullable | Default | Описание |
|---|---|---|---|---|
| `id` | UUID PK | нет | `gen_random_uuid()` | — |
| `email` | TEXT | нет | — | основной канал, нормализованный (lowercase/trim) |
| `display_name` | TEXT | да | `NULL` | имя, вводимое самим пользователем (не обязательно паспортное ФИО — как никнейм); `NULL` отображается как «Родитель» по умолчанию у отзыва |
| `locale` | TEXT | нет | `'ru'` | `ru`\|`tg` — для локализации уведомлений (push/email) конкретному пользователю |
...
| `status` | TEXT | нет | `'unverified'` | `unverified`\|`active`\|`banned`\|`deleted` (CHECK) — `deleted` для soft-delete (см. ниже) |
...
| `notification_channels` | JSONB | нет | `'{"push":true,"email":true}'` | простой набор переключателей канала уведомлений — не история, поэтому не отдельная таблица |
...
**Связи:** родитель для `refresh_tokens`, `children`, `oauth_identities`, `verification_codes`, `employment_claims`, и (по `user_id`, без физического FK за пределами схемы) для владельцев/акторов во всех остальных схемах.
**Индексы:** `UNIQUE(email)`; частичный `UNIQUE(phone) WHERE phone IS NOT NULL`.

**Примечание к RBAC:** «родитель» и «соискатель» — не роли, а факты о пользователе (наличие записей в `children`, значение `applicants.visibility`), уже так реализовано в `web/lib/app-state.tsx`. `role` в этой таблице — только уровень доступа.
...
**Индексы:** `UNIQUE(user_id, institution_id)` — защита от самонакрутки: без этого ограничения пользователь мог бы создать множество привязок к одному учреждению и обойти анти-фрод логику отзывов (FR-15/FR-18); `btree(institution_id, confirmation_status)` (для очереди подтверждения в кабинете учреждения и для eligibility-проверки отзыва в вехе 4 через порт `auth.ChildLinkExists`, которая проверяет `confirmation_status='confirmed'`, не только факт существования строки).
...
| `description` | JSONB | да | `NULL` | `{ru,tg}` — полное описание (FR-06: всё нужное для решения родителя, не переходя на внешние сайты) |
...
| `plan` | TEXT | нет | `'free'` | `free`\|`pro`\|`enterprise` — влияет только на бизнес-лимиты кабинета учреждения (лимиты галереи/вакансий/сотрудников, глубина аналитики, приоритет верификации — см. `docs/EduHub_Pricing_Tiers.md`), никогда на позицию в поиске или на `rating_avg` |
...
Денормализация 8 метрик рейтинга (не только общего `rating_avg`) — карточка учреждения (`GetByID`, задача 24 плана) собирается одним SQL-запросом и должна отдавать разбивку по метрикам, а `catalog` не вправе читать чужую схему `reviews` напрямую. Пишет только `catalog` через порт `RatingSync.Apply(ctx, instID, metrics []MetricScore)`, источник расчёта — веха 4 (`reviews.institution_rating_agg`).
...
| `text` | TEXT | нет | — | свободный текст на любом языке, независимо от языка интерфейса платформы — отзыв пишет живой человек в моменте на одном языке, не переводит сам себя на оба; тот же принцип, что у `communications.messages.body` |
| `reply` | TEXT | да | `NULL` | ответ учреждения (FR-17), тот же принцип |
| `status` | TEXT | нет | `'pending'` | `pending`\|`approved`\|`rejected`\|`disputed`\|`resolved_kept`\|`resolved_removed` — спор разрешается в одно из двух явных конечных состояний, не остаётся в `disputed` |
...
| `metric_key` | TEXT | нет | — | `quality`\|`conditions`\|`safety`\|`food`\|`transport`\|`price`\|`parent_involvement`\|`inclusivity` (CHECK, 8 значений) |
...
Отзыв сотрудника (текущего/бывшего) об учреждении как о работодателе (FR-41) — полностью отдельный контур от родительских отзывов: верификация через модератора (не через `Child`), полностью отдельные метрики (не влияют на `catalog.institution_metrics`/`rating_avg`), видимость ограничена соискателями (см. ниже) — учреждение как платящий клиент никогда не видит это на своей публичной родительской витрине.
...
| `text` | TEXT | нет | — | свободный текст — пишет один человек на одном языке |
...
| `actor_type` | TEXT | нет | `'user'` | `user`\|`system` (CHECK) — системные действия (авто-флаг велосити-детектора FR-18, автовозврат claim по таймауту, ночные джобы) не имеют человека-актора |
...
**Инвариант:** запись создаётся только через `Recorder.Record(ctx, tx, Entry)`, принимающий `pgx.Tx` — физически невозможно записать вне транзакции самого изменения.
...
**Индексы:** частичный `btree(priority DESC, created_at) WHERE status='pending'`; `btree(claimed_by, claimed_at) WHERE status='claimed'` (для таймаут-джоба возврата в пул); частичный `UNIQUE(target_type, target_id) WHERE status <> 'resolved'` — без него повторная постановка той же цели создавала бы второй айтем, и два модератора могли бы claim'ить один `target_id` через два разных `queue_items.id`, обходя саму цель claim-паттерна; повторный флаг цели анти-спам детектором — `UPDATE` существующего pending-айтема, не `INSERT`.
...
Диалог (FR-19) — не только «родитель ↔ учреждение», но и «родитель ↔ родитель»: оба участника — полиморфные слоты, каждый может быть либо пользователем, либо учреждением. Раздельные `*_last_read_at` по сторонам, поскольку прочтение одной стороной не означает прочтение другой.
...
**Индексы:** `UNIQUE(participant_a_type, participant_a_id, participant_b_type, participant_b_id)` — при создании диалога usecase канонизирует порядок участников (например сортировкой по `type`, затем по `id`) до вставки, чтобы пара «X↔Y» и «Y↔X» не создали два разных диалога — канонизация в usecase, не в БД.
...
Заявка на визит в учреждение (форма уже есть на фронте, `web/app/(site)/institutions/[id]/page.tsx:344-352`; `.claude/rules/architecture.md` называет её среди мутаций, требующих идемпотентности).
...
**Индексы:** `btree(institution_id, status, created_at)`; частичный `UNIQUE(institution_id, phone) WHERE status='new'` — защита от дубль-клика (повторная заявка на ту же институцию с тем же телефоном пока предыдущая ещё не обработана не создаёт вторую запись).
...
Дедупликация at-least-once доставки Redis Streams. Нужна именно здесь (не в `reviews`), потому что `RatingSync.Apply` пишет абсолютные значения (текущее взвешенное среднее, не инкремент) и естественно идемпотентен без dedup-стора — а уведомления (push/in-app) не идемпотентны по построению: повторная доставка того же `event_id` реально отправит пользователю второй push о том же событии.
...
Web Push подписки (Push API + Service Worker, встроенный браузерный стандарт — не внешний платный провайдер, в отличие от SMS/email). Один пользователь может иметь несколько подписок (несколько устройств/браузеров).
...
Достижения соискателя (`web/lib/data.ts:2002`, `Applicant.achievements`). Не переиспользует полиморфную `catalog.achievements` (та в схеме `catalog`, `communications` не может писать в чужую таблицу — правило владения схемами); структура продублирована, тот же паттерн, что у `reviews.employer_review_metrics` отдельно от `reviews.review_metrics`.

| Поле | Тип | Nullable | Default | Описание |
|---|---|---|---|---|
| `id` | UUID PK | нет | `gen_random_uuid()` | — |
| `applicant_id` | UUID FK→`communications.applicants(id)` ON DELETE CASCADE | нет | — | — |
| `title` | TEXT | нет | — | свободный текст (не `JSONB{ru,tg}` — пишет сам соискатель, тот же принцип, что `reviews.reviews.text`) |
| `year` | INT | да | `NULL` | — |
...
Обращение учреждения к соискателю напрямую (обратное направление к `applications`).
...
| `institution_id` | UUID | нет | — | по значению |
| `applicant_id` | UUID FK→`communications.applicants(id)` ON DELETE CASCADE | нет | — | — |
| `message` | TEXT | нет | — | свободный текст — пишет живой человек, тот же принцип что `reviews.reviews.text` |
| `created_at` | TIMESTAMPTZ | нет | `now()` | — |

**Связи:** N—1 `communications.applicants`.
**Индексы:** `btree(institution_id, applicant_id, created_at)`. **Без постоянного `UNIQUE(institution_id, applicant_id)`**: FR-37 требует лимит обращений за период, не «одно обращение навсегда» — постоянный `UNIQUE` мешал бы легитимному повторному обращению через год по другой вакансии. Идемпотентность одного конкретного запроса (защита от дубль-клика) — через `Idempotency-Key` (`platform.idempotency_keys`). Анти-спам лимит за период (FR-37) — Redis-счётчик в usecase, не constraint в БД (скользящее окно «последние N дней» неудобно выразить постоянным индексом).
...
Сырые события просмотра/клика. Партиционирована по месяцам (`PARTITION BY RANGE(occurred_at)`) — retention 90 дней снимается через `ALTER TABLE ... DETACH PARTITION` + `DROP TABLE` на отсоединённую партицию, не построчный `DELETE`. Новые партиции создаёт фоновый джоб заранее (`cmd/worker`, план E6.3.5) — без него вставка упадёт в момент перехода на новый месяц.
...
| `referrer_class` | TEXT | нет | — | `search`\|`catalog`\|`map`\|`direct`\|`external` (CHECK, FR-22/23 «источники трафика») — классифицируется на бэкенде из `Referer`-заголовка при записи события, сырой URL не сохраняется (тот же принцип, что `user_agent_class`: полный `Referer` может содержать PII в query-параметрах источника) |
...
**Релевантные файлы:** полный план реализации и обоснование решений — `docs/EduHub_Backend_Development_Plan.md`. Состав тарифов — `docs/EduHub_Pricing_Tiers.md`. Контракты фронта для сверки полей — `web/lib/data.ts`, `web/lib/app-state.tsx`.
