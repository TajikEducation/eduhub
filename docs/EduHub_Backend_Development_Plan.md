# EduHub — план разработки бэкенда
...
- Двуязычность ru/tg — архитектурно, с первого дня: JSONB `{ru,tg}` для контента, который платформа/учреждение готовит заранее (симметрично `Bi` из `web/lib/data.ts:23`); для текста, который пишет живой человек в моменте (отзыв, сообщение в чате) — обычный `TEXT`, на любом языке, независимо от языка интерфейса.
- **Минимизация PII детей**: `auth.children` хранит только `age_group` + `status` + `institution_id` + `confirmation_status`. Имя/фото ребёнка бэкенд не принимает, не хранит, не логирует — ломает текущий фронтовый `ChildLink.name`, правка фронта нужна отдельно (см. Risks).
...
2. **`auth.children.institution_id` — обычный физический FK на `catalog.institutions(id)`**, `ON DELETE RESTRICT` (институция с привязанными детьми не может быть физически удалена — только смена `moderation_status`, soft-архивация; институции в этом продукте не удаляются физически ни при каком сценарии MVP). Это единственное исключение из правила «кросс-схемные FK только на `user_id`» — оправдано целостностью данных, влияющих на верификацию отзывов (FR-15). Миграционный порядок: `catalog.institutions` создаётся в вехе 1 (веха, предшествующая auth), поэтому к моменту миграции `auth.children` в вехе 2 таблица-цель уже существует — проблемы очерёдности миграций нет.
...
**Rate-limiting — покрытие по эндпоинтам.** Механизм спроектирован один раз в вехе 0 (задача 15, in-memory token bucket по IP, Redis-based версия — веха 5 при мультиинстансе) и применяется единообразно ко всем точкам, принимающим пользовательский ввод:

| Эндпоинт | Ключ лимита | Обоснование |
|---|---|---|
| `/auth/login` | email+IP | anti-bruteforce (E2.7) |
...
| `POST` создание нового диалога (`communications.conversations`) | `user_id` за период | отдельно от лимита сообщений — защита от «написать сразу многим одно и то же», применяется одинаково независимо от типа собеседника (пользователь или учреждение) |
...
| `POST` создание отзыва | `user_id`+IP | FR-18 |
...
| `POST /communications/employer_responses` | `institution_id` за период | FR-37 — Redis-счётчик за период, не постоянный БД-constraint |

**Версионирование API.** Минимальная политика — не избыточная инженерия под потребителей, которых пока нет:
- `v1` — единственная версия на MVP, ничего параллельно не запускаем (один контролируемый фронтенд-клиент).
- Breaking change **до первого продакшн-релиза** — правим напрямую внутри `v1`, версионирование не нужно (внешних потребителей ещё нет).
- **После первого продакшн-релиза** — предпочтение additive-изменениям (новое опциональное поле, не трогая старое). Если breaking неизбежен — `v2` только для конкретного изменившегося ресурса (не всего API), `v1` этого ресурса живёт минимум 30 дней со стандартными HTTP-заголовками `Deprecation`/`Sunset` (`.claude/rules/go.md`: «deprecate перед удалением»).
- Явно НЕ делаем: content negotiation по заголовку, GraphQL-подобную эволюцию схемы — избыточно для одного клиента.

---

## Модерация — дизайн

Модерируются: регистрация учреждения (pending→approved/rejected), отзывы (FR-16, SLA ≤24-48ч), споры по отзывам (FR-17/FR-35, эскалация, SLA ≤72ч), верификация владельца профиля (FR-34), верификация трудоустройства для отзывов о работодателе (FR-41).

**Основные решения (MVP, простота приоритет):**
...
- **Отказ в регистрации учреждения — без формального пути апелляции.** Учреждение правит данные и подаёт заявку заново. Не симметрично dispute-процессу у отзывов (тот — по FR-35, обязателен).
- **Одна общая очередь модерации** на всех модераторов — без деления по региону/фильтрам.

**Практики, входящие в дизайн по умолчанию:**
- **Explicit state machine**, не булевы флаги: `institution.moderation_status: pending→approved|rejected`; `review.status: pending→approved|rejected`, отдельно `disputed→resolved_kept|resolved_removed`. Переход — только через один код-путь (transition guard в usecase), не произвольный UPDATE.
- **Claim-паттерн** вместо голого FIFO: модератор «берёт в работу» айтем (`claimed_by`, `claimed_at` — nullable колонки на очереди), таймаут (например 15 минут) возвращает заброшенный айтем в пул.
- **Причина reject/dispute-resolution — обязательна и структурирована**: enum (`no_license`/`duplicate`/`fake_suspected`/`other`) + свободный текст. Нужно для аналитики причин отказов.
...
Что: `postgis/postgis:16-3.4` (порт 5433), `redis:7-alpine`, `minio/minio` (S3-совместимое хранилище — **только dev/test**, в проде реальный S3-совместимый провайдер по конфигу; нужен уже с вехи 3, где загрузка медиа идёт через multipart-эндпоинт бэкенда — без него интеграционные тесты загрузки не на чем гонять), healthcheck, init-скрипт создаёт `eduhub_test`.
...
Контракт эндпоинтов:
- `GET /api/v1/institutions?q&type&region&area&min_price&max_price&min_rating&transport&food&verified&curriculum&program_level&discount&sort&lat&lng&radius_km&limit&cursor`
...
RED: (а) `GetByID` собирает staff/achievements/gallery/alumni/transport_routes/meal_plans одним вызовом; (б) отсутствует → `NotFound`; (в) без сотрудников → `Staff=[]`, не `nil`; (г) NULL в `license_no`/`price` → `nil`, не `""`/`0`; (д) счётчик SQL-запросов = 1 (через pgx QueryTracer — защита от N+1 в будущих рефакторингах); (е) элементы каждой коллекции (`gallery`, `transport_routes`, `meal_plans` и т.д.) возвращаются в стабильном порядке между повторными вызовами.
...
**E2.1 Схема `auth`.** `users` (`email` уникальный основной канал, `phone` nullable опциональный второй очереди, `password_hash` nullable — пусто для чисто-Google-аккаунтов, `role`, `status ∈ {unverified,active,banned,deleted}`, `email_verified_at`, `consent_at`+`consent_version`, `display_name`, `locale`, `notification_channels`, `failed_login_count`, `locked_until`, `deleted_at`), `oauth_identities` (`provider='google'`, `provider_user_id`, UNIQUE пара), `verification_codes` (`channel`, `purpose`, `code_hash`, `attempts_count`, `expires_at`), `refresh_tokens` (`token_hash`, `family_id`, `expires_at`, `revoked_at`, `replaced_by`), `children` (`user_id`, `institution_id` **FK на `catalog.institutions` ON DELETE RESTRICT** — см. Architecture, `age_group`, `status ∈ {current,alumnus,transferred}`, `confirmation_status ∈ {pending,confirmed,rejected}` + `confirmed_by`/`confirmed_at`, БЕЗ имени/фото, `UNIQUE(user_id,institution_id)`). `UNIQUE` ограничивает только повтор одной и той же пары — один пользователь может иметь несколько привязок к разным учреждениям одновременно (ребёнок ходит в школу и параллельно в доп.центр — обе привязки `current` в одно и то же время), включая параллельные привязки с разными статусами.

**Приоритет каналов регистрации:** email/пароль + вход через Google — основные равноправные пути. Телефон — опциональное поле второй очереди, без обязательной SMS-верификации (SMS-провайдер в РТ платный за отправку, не оправдан на холодном старте — email не требует внешнего платного канала).
...
**E2.6 Children CRUD с PII-минимизацией и подтверждением учреждением.** API принимает только `institution_id`, `age_group`, `status`. Явный тест: поле `name` в запросе → 400 `unknown field`. **Проверка статуса институции — явный guard в usecase, не FK**: FK на `catalog.institutions` гарантирует только существование строки (ссылочную целостность), не значение `moderation_status` — привязка к `pending`/`rejected` учреждению технически пройдёт FK, поэтому usecase обязан отдельно проверить `moderation_status='approved'` через порт `catalog.InstitutionStatus` до вставки. Привязка создаётся со `confirmation_status='pending'`; отдельный эндпоинт в кабинете учреждения — `GET /institutions/{id}/children/pending` (роль `institution`, свой институт) + `POST /children/{id}/confirm|reject` (структурированная причина при reject, запись в `moderation.audit_log`). Подтверждает учреждение по личности родителя (email/имя, указанные при регистрации — это ПД самого родителя, не ребёнка), не по данным о ребёнке. Право на отзыв (веха 4, E4.2) требует `confirmed`, не просто существования строки.
...
**E3.8 Тарифы и лимиты.** `POST /api/v1/admin/institutions/{id}/plan` (роль `admin`) — назначение/продление тарифа учреждению, с записью в `moderation.audit_log` (это деньги — должно быть прослеживаемо, как остальные решения администратора). `plan_expires_at` истекает — учреждение автоматически возвращается на лимиты Free (проверка в usecase при каждом обращении к лимитируемому ресурсу, отдельного джоба не требует). Полный состав тарифов (лимиты галереи/вакансий/сотрудников/новостей, глубина аналитики, приоритет верификации, лимит обращений к соискателям) — `docs/EduHub_Pricing_Tiers.md`. Позиция учреждения в результатах поиска и его рейтинг от тарифа не зависят ни при каких условиях.

**Критерии готовности:** ни один **модераторский** мутирующий эндпоинт (`/moderation/*`, назначение тарифа, owner-verification approve/reject) не проходит тест без записи в `moderation.audit_log` — обычные пользовательские действия (создание `ChildLink`, отправка отзыва) audit-записи не требуют: `audit_log` — журнал решений модерации, не общий лог всех действий; policy-тест по всем зарегистрированным модераторским роутам. Повторный POST с тем же `Idempotency-Key` не создаёт дубль.

---

### ВЕХА 4 — Reviews / Ratings

**E4.1 Схема `reviews`.** `reviews` (UNIQUE(`user_id`,`institution_id`,`child_id`) — идемпотентность на уровне БД, `verified_at_publish` — снимок статуса верификации на момент публикации, см. E4.7), `review_metrics` (нормализовано по строкам: `review_id`,`metric_key`,`score 1-5`, UNIQUE(`review_id`,`metric_key`)), `institution_rating_agg` (`institution_id`,`metric_key`,`weighted_avg`,`review_count`,`updated_at`). Отдельным контуром (см. E4.9) — `employer_reviews`+`employer_review_metrics` для отзывов о работодателе.
**8 метрик**: `quality, conditions, safety, food, transport, price, parent_involvement, inclusivity`. Общий рейтинг — вычисляемое среднее по этим восьми, не отдельно оцениваемая величина.

**E4.2 Eligibility (верификация отзыва).** Разрешено только при наличии `auth.children` со связью user→institution в статусе `current`/`alumnus`/`transferred` **и** `confirmation_status='confirmed'` — самодекларации родителя недостаточно, подтверждение учреждением (E2.6) — обязательная вторая линия защиты сверх `UNIQUE(user_id,institution_id)`. Проверка — в usecase через порт `auth.ChildLinkExists` (кросс-схемная логика — не через FK, поскольку `reviews` не владеет схемой `auth`).
...
**E4.5 Синхронизация с каталогом.** Консьюмер вызывает порт `catalog.RatingSync.Apply(ctx, instID, metrics []MetricScore)` — передаёт массив всех 8 метрик, не только общее среднее: карточка учреждения рендерит разбивку по метрикам, `catalog` не вправе читать схему `reviews` напрямую, поэтому нужен полный набор через порт. `catalog` сам пишет в свои денормализованные колонки — `catalog.institutions.rating_avg`/`review_count` (общее) + `catalog.institution_metrics` (по каждой из 8 метрик) — бампает cache version. Владение схемами соблюдено. **Идемпотентность естественная**: `Apply` пишет абсолютные значения (текущее взвешенное среднее), не инкремент — повторная доставка того же события Redis Streams (at-least-once) даёт тот же результат при повторном применении, отдельный dedup-стор здесь не нужен (в отличие от `communications`-консьюмера уведомлений, см. E5.3, где доставка не идемпотентна по построению).
...
**E4.7 Модерация отзывов и споры.** Статусы `pending/approved/rejected/disputed→resolved_kept|resolved_removed`, ответ учреждения (`reply`), формальный спор с SLA 72ч (`dispute_deadline`, `disputed_by`, `dispute_reason`, джоб-эскалация) — здесь путь апелляции есть (в отличие от отказа в регистрации учреждения, см. раздел «Модерация» выше). **Критерий разрешения спора сверяется со снимком, не с текущим состоянием**: при публикации отзыва (E4.2) в строку копируется `verified_at_publish` — было ли `confirmation_status='confirmed'` в тот момент. Иначе разрешение спора зависело бы от состояния `auth.children`, которое могло измениться уже после публикации (родитель удалил привязку, учреждение отозвало подтверждение), что несправедливо по отношению к обеим сторонам. Каждое действие — в `moderation.audit_log`.
...
**E4.9 Отзывы об учреждении как о работодателе.** Полностью отдельный контур от родительских отзывов — своя верификация, свои метрики, ограниченная видимость (FR-41):
...
- **5 метрик** (нормализовано по строкам, как `review_metrics`): `salary_conditions`, `management`, `team_atmosphere`, `workload`, `professional_growth`. Не влияют на `catalog.institution_metrics`/`rating_avg` — полностью изолированная агрегация.
...
- **Видимость жёстко ограничена**: `GET /api/v1/institutions/{id}/employer-reviews` требует авторизации И наличия у пользователя записи `communications.applicants` (то есть он соискатель). Родители, гости и обычные пользователи без анкеты соискателя не видят ни на публичном профиле учреждения, ни по прямой ссылке — платящее учреждение никогда не видит эти отзывы на своей родительской витрине.
- Идемпотентность — `UNIQUE(user_id, institution_id)` на `employer_reviews`, один отзыв о работодателе на пару.

**Критерии готовности:** тест формулы decay на фиксированных часах (`platform/clock`); тест «повторная доставка события не искажает агрегат»; тест «институция с 3 отзывами → `rating_status=insufficient`»; перф-замер ночного пересчёта — прогон на синтетических 10-50 тыс. отзывов, длительность зафиксирована в `perf-baseline.md` (при `pg_try_advisory_lock` долгий пересчёт держит лидерство долго — нужно знать порядок величины заранее, не выяснять на проде).

---

### ВЕХА 5 — Communications

**E5.1 Схема `communications`.** `conversations` (полиморфные участники `participant_a/b_type ∈ {user,institution}` — диалог не только родитель↔учреждение, но и родитель↔родитель, `id` = `chat:conv:{id}`, раздельные `participant_a/b_last_read_at`) + `messages` (`conversation_id FK`, `sender_type`, `sender_id`); `notifications`; `push_subscriptions` (Web Push); `processed_events` (дедупликация at-least-once доставки уведомлений); `vacancies`; `applicants`; `applicant_achievements`; `applications` UNIQUE(`applicant_id`,`vacancy_id`) — идемпотентность на уровне БД; `employer_responses` — идемпотентность конкретного запроса через `Idempotency-Key`, анти-спам лимит за период (FR-37) через Redis-счётчик в usecase (не постоянный `UNIQUE`, который навсегда запрещал бы учреждению повторно написать тому же соискателю); `visit_requests` (форма уже есть на фронте, `web/app/(site)/institutions/[id]/page.tsx:344-352`; идемпотентно через частичный `UNIQUE(institution_id,phone) WHERE status='new'`, обязателен rate-limit по IP+телефону — публичный эндпоинт для гостя собирает PII). Полный список полей — `docs/EduHub_Database_Schema.md`.

**E5.2 WebSocket-хаб и мультиинстансность.** Соединения в памяти инстанса; кросс-инстансная доставка — Redis Pub/Sub (`chat:conv:{id}`, backed реальной сущностью `communications.conversations.id`). Сообщение сначала пишется в Postgres (источник истины), затем публикуется. Клиент дедуплицирует по `message_id` (= `messages.id`), история — REST-запросом, WS только за «живую» доставку. N инстансов без sticky-сессий. Rate-limit на отправку сообщения — по `sender_id`, защита от спама в чат; отдельный rate-limit на **создание нового диалога** (не на сообщение внутри уже существующего), применяется одинаково независимо от типа собеседника (пользователь или учреждение) — тот же спам-вектор «написать сразу многим одно и то же». Механизм обнаружения (как пользователь находит `user_id` другого родителя, чтобы начать диалог) — открытый вопрос, требующий отдельного продуктового решения до реализации; не блокирует саму схему.
...
**E5.3 Уведомления.** Redis Streams + consumer group. Адаптеры провайдеров за интерфейсом `notify.Sender`. Push в браузере/PWA — реальный канал MVP, не заглушка: Web Push API — встроенный браузерный стандарт через VAPID-ключи, бесплатный, не требует внешнего платного провайдера в отличие от SMS-агрегаторов в РТ; NFR CLAUDE.md называет push основным каналом PWA. SMS/email остаются заглушками с логированием (провайдер для РТ не выбран — здесь задержка обоснована). `communications.push_subscriptions` хранит Web Push подписки (endpoint+ключи), `POST/DELETE /api/v1/push/subscribe`, `auth.users.notification_channels` — переключатели канала по пользователю. Ретраи с backoff, DLQ-стрим после N попыток. **Дедупликация at-least-once**: в отличие от `RatingSync` (естественно идемпотентен, см. E4.5), доставка push/in-app не идемпотентна по построению — повторная доставка того же события реально отправит второй push. `communications.processed_events(event_id PK)`, консьюмер сначала `INSERT` — конфликт означает «уже обработано», пропустить.
...
**Критерии готовности:** интеграционный тест с двумя экземплярами хаба на одном Redis (сообщение из хаба A доходит до клиента хаба B); тест на отсутствие утечки горутин; тест «повторный apply не создаёт второй `application`»; soak-тест WS — не менее 30 минут под несколькими сотнями одновременных соединений, слежение за `NumGoroutine()`/памятью процесса (короткий тест на 100 соединений ловит очевидные утечки, но не накопление за время, характерное для long-running WS-хаба).

---

### ВЕХА 6 — Analytics

**E6.1 Схема `analytics`.** `profile_events` — партиции по месяцам (`PARTITION BY RANGE(occurred_at)`), поля включают `ip_hash` (хешированный, не сырой IP — PII), `user_agent_class` (bot/mobile/desktop, не полная строка), `referrer_class` (`search`/`catalog`/`map`/`direct`/`external` — классификация `Referer`-заголовка на handler'е, сырой URL не сохраняется, поскольку может нести PII в query источника — та же логика, что у `user_agent_class`, нужна для FR-22/23 «источники трафика»). `profile_events_daily` — агрегат с PK для идемпотентного upsert.
...
**E6.3.5 Управление партициями.** Джоб в `cmd/worker` (тот же контур, что ночной пересчёт рейтинга/агрегация) — создаёт партицию `analytics.profile_events` на следующий месяц заранее (в начале текущего месяца — партицию на месяц вперёд, запас на случай если джоб один раз не отработает), `CREATE TABLE ... PARTITION OF ... FOR VALUES FROM (...) TO (...)` идемпотентно (проверка существования перед созданием). Без этого джоба вставка упадёт с ошибкой ровно в момент перехода на новый месяц — нет `DEFAULT`-партиции, ловящей всё, что не попало ни в одну явную.

**E6.4 Retention.** Сырые события — 90 дней, удаление через `ALTER TABLE ... DETACH PARTITION` + `DROP TABLE` на отсоединённую партицию — мгновенно, без блокировок и раздувания WAL, в отличие от построчного `DELETE` на большом объёме.
...
**Критерии готовности:** нагрузочный тест — `hey`-прогон каталога (тот же сценарий что задача 31) с одновременно активным batch-writer'ом аналитики под нагрузкой, конкретной командой подтверждающий, что включение аналитики не сдвигает p95 каталога; тест «переполнение канала не блокирует handler»; в `profile_events` нет сырых IP/телефонов (тест схемы).
...
- `Institution.type` — на фронте одиночный `CategoryKey`, в БД `TEXT[]` (мульти-тип). API отдаёт `types: string[]`, фронт до адаптации берёт `types[0]`.
...
**7. Audit trail с дырами.** Митигация: `Recorder.Record` принимает `pgx.Tx`, не пул — сигнатура физически не даёт записать вне транзакции. Policy-тест из вехи 3 перебирает все модераторские мутирующие роуты.

**8. At-least-once доставка искажает агрегаты рейтинга.** Митигация: `RatingSync.Apply` пишет абсолютные значения — естественно идемпотентен, отдельный dedup-стор для этого консьюмера не нужен; ночной полный пересчёт — самовосстановление от накопленных расхождений. Для `communications`-консьюмера (не идемпотентен по построению) — `processed_events(event_id PK)`, INSERT-based dedup.
...
- **Наблюдаемость** (Prometheus/Grafana, RED-метрики, `/metrics`, алертинг). Без стейджинга/прода и реального трафика метрики некому скрейпить и не на что алертить — мёртвая инфраструктура на MVP. NFR (p95≤300мс) на вехах 0-5 проверяется вручную (`hey`-прогон, задача 31 и её аналоги на других вехах), не постоянным мониторингом.
