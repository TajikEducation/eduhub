...
| `POST /communications/employer_responses` | `institution_id` за период | FR-37 — Redis-счётчик за период (не постоянный БД-constraint — см. находку про пожизненную уникальность, закрыто) |

**Версионирование API (добавлено 2026-08-25).** Минимальная политика — не избыточная инженерия под потребителей, которых пока нет:
...
Приёмка: `go test -tags=integration ./cmd/devseed/ -v` → PASS; `make seed && curl ... | jq '.items|length'` → `9`. **CI-шаг миграций на заполненной БД** (добавлено 2026-08-25) — с этой задачи в CI появляется первый реальный сид, добавить прогон `migrate-up → seed → migrate-down → migrate-up` (не только на пустой схеме, как в задаче 11/17-19) — ловит миграции, технически валидные на пустой БД, но потенциально падающие на заполненной.
...
**E3.1 Пакет `internal/moderation`.** Схема `moderation.audit_log` (`actor_type ∈ {user,system}`, `actor_id NULL` для системных действий, `actor_role`, `action`, `target_type`, `target_id`, `reason_code`/`reason_text`, `payload_diff JSONB`, `request_id`, `created_at` — актуализировано под `docs/EduHub_Database_Schema.md`). Порт `moderation.Recorder.Record(ctx, tx, Entry) error` — принимает транзакцию (запись audit физически невозможна вне транзакции изменения). Здесь же — очередь модерации с claim-паттерном (`claimed_by`, `claimed_at`, `priority`, `flagged_reason`) — см. раздел «Модерация — детальный дизайн» выше.
...
**Критерии готовности:** ни один **модераторский** мутирующий эндпоинт (`/moderation/*`, назначение тарифа, owner-verification approve/reject) не проходит тест без записи в `moderation.audit_log` (сужено 2026-08-25 — было «любой мутирующий эндпоинт», что физически невыполнимо для обычных пользовательских действий вроде добавления `ChildLink`, `audit_log` — журнал решений модерации, не общий лог всех действий; policy-тест по всем зарегистрированным **модераторским** роутам); повторный POST с тем же `Idempotency-Key` не создаёт дубль.
...
**E4.5 Синхронизация с каталогом.** Консьюмер вызывает порт `catalog.RatingSync.Apply(ctx, instID, metrics []MetricScore)` — **передаёт массив всех 8 метрик**, не только общее среднее (исправлено 2026-08-25: карточка учреждения рендерит разбивку по 8 метрикам, одного `avg` недостаточно для собственного одного-запросного чтения карточки; `catalog` не вправе читать схему `reviews` напрямую). `catalog` сам пишет в свои денормализованные колонки — `catalog.institutions.rating_avg`/`review_count` (общее) + `catalog.institution_metrics` (по каждой из 8 метрик, новая сателлит-таблица) — бампает cache version. Владение схемами соблюдено. **Идемпотентность естественная** (уточнено 2026-08-25): `Apply` пишет **абсолютные значения** (текущее взвешенное среднее), не инкремент — повторная доставка того же события Redis Streams (at-least-once) даёт тот же результат при повторном применении, отдельный dedup-стор здесь не нужен (в отличие от `communications`-консьюмера уведомлений, см. E5.3 — там доставка не идемпотентна по построению).
...
**Критерии готовности:** тест формулы decay на фиксированных часах (`platform/clock`); тест «повторная доставка события не искажает агрегат»; тест «институция с 3 отзывами → `rating_status=insufficient`»; **перф-замер ночного пересчёта** (добавлено 2026-08-25) — прогон на синтетических 10-50 тыс. отзывов, длительность зафиксирована в `perf-baseline.md` (при `pg_try_advisory_lock` долгий пересчёт держит лидерство долго — нужно знать порядок величины заранее, не выяснять на проде).

---

### ВЕХА 5 — Communications

**E5.1 Схема `communications`.** 10 таблиц (актуализировано 2026-08-25, см. `docs/EduHub_Database_Schema.md` для полного списка полей): `conversations` (полиморфные участники user/institution, `id` = `chat:conv:{id}`, раздельные `participant_a/b_last_read_at`) + `messages` (`conversation_id FK`, `sender_type`, `sender_id`); `notifications`; `push_subscriptions` (Web Push); `vacancies`; `applicants`; `applicant_achievements`; `applications` UNIQUE(`applicant_id`,`vacancy_id`) — идемпотентность на уровне БД; `employer_responses` — **без постоянного UNIQUE** (было `UNIQUE(institution_id,applicant_id)`, исправлено — путало идемпотентность одного запроса, теперь через `Idempotency-Key`, с FR-37 анти-спам лимитом за период, теперь через Redis-счётчик в usecase); `visit_requests` (форма уже есть на фронте, `web/app/(site)/institutions/[id]/page.tsx:344-352`, не было ни таблицы, ни эндпоинта; идемпотентно через частичный `UNIQUE(institution_id,phone) WHERE status='new'`, обязателен rate-limit по IP+телефону — публичный эндпоинт для гостя собирает PII).
...
**E5.3 Уведомления.** Redis Streams + consumer group. Адаптеры провайдеров за интерфейсом `notify.Sender`. **Пересмотрено 2026-08-25**: push в браузере/PWA — реальный канал в MVP, не заглушка (Web Push API — встроенный браузерный стандарт через VAPID-ключи, бесплатно, не требует внешнего платного провайдера в отличие от SMS; NFR CLAUDE.md называет push основным каналом PWA — откладывать вместе с SMS/email было ошибкой плана, не осознанным решением). SMS/email остаются заглушками с логированием (провайдер для РТ не выбран — здесь задержка обоснована). `communications.push_subscriptions` хранит Web Push подписки (endpoint+ключи), `POST/DELETE /api/v1/push/subscribe`, `auth.users.notification_channels` — переключатели канала по пользователю. Ретраи с backoff, DLQ-стрим после N попыток. **Дедупликация at-least-once** (добавлено 2026-08-25 — в отличие от `RatingSync`, доставка push/in-app не идемпотентна по построению: повторная доставка того же события реально отправит второй push): `communications.processed_events(event_id PK)`, консьюмер сначала `INSERT` — конфликт означает «уже обработано», пропустить.
...
**Критерии готовности:** интеграционный тест с двумя экземплярами хаба на одном Redis (сообщение из хаба A доходит до клиента хаба B); тест на отсутствие утечки горутин; тест «повторный apply не создаёт второй `application`»; **soak-тест WS** (добавлено 2026-08-25) — не менее 30 минут под несколькими сотнями одновременных соединений, слежение за `NumGoroutine()`/памятью процесса — короткий тест на 100 соединений ловит очевидные утечки, но не накопление за время, характерное именно для long-running WS-хаба.
...
**E6.3.5 Управление партициями (добавлено 2026-08-25).** Джоб в `cmd/worker` (тот же контур, что ночной пересчёт рейтинга/агрегация) — создаёт партицию `analytics.profile_events` на следующий месяц заранее (в начале текущего месяца — партицию на месяц вперёд, запас на случай если джоб один раз не отработает), `CREATE TABLE ... PARTITION OF ... FOR VALUES FROM (...) TO (...)` идемпотентно (проверка существования перед созданием). Без этого джоба вставка упадёт с ошибкой ровно в момент перехода на новый месяц — нет `DEFAULT`-партиции, ловящей всё, что не попало ни в одну явную.

**E6.4 Retention.** Сырые события — 90 дней, удаление через **`ALTER TABLE ... DETACH PARTITION` + `DROP TABLE`** на отсоединённую партицию (исправлено 2026-08-25 — `DROP PARTITION` не существующий синтаксис PostgreSQL) — мгновенно, без блокировок и раздувания WAL, в отличие от построчного `DELETE` на большом объёме.
...
**Критерии готовности:** нагрузочный тест — **конкретно** `hey`-прогон каталога (тот же сценарий что задача 31) с одновременно активным batch-writer'ом аналитики под нагрузкой — подтверждает, что включение аналитики не сдвигает p95 каталога (уточнено 2026-08-25 — раньше формулировка была словами без конкретной команды/числа); тест «переполнение канала не блокирует handler»; в `profile_events` нет сырых IP/телефонов (тест схемы).
...
**8. At-least-once доставка искажает агрегаты рейтинга.** Митигация: `RatingSync.Apply` пишет абсолютные значения (уточнено 2026-08-25 — естественно идемпотентен, не нужен отдельный dedup-стор для этого консьюмера конкретно); ночной полный пересчёт — самовосстановление от накопленных расхождений. Для `communications`-консьюмера (не идемпотентен по построению) — `processed_events(event_id PK)`, INSERT-based dedup.
...
| 37 | Версионирование API (`/api/v1`) без политики breaking change — `.claude/rules/go.md` требует deprecate перед удалением, нечем исполнить | Минимальная политика: `v1` единственная на MVP, до прод-релиза правим напрямую, после — additive-приоритет или `v2` на конкретный ресурс с `Deprecation`/`Sunset`-заголовками, минимум 30 дней |
| 38 | `employer_responses` — постоянный `UNIQUE(institution_id,applicant_id)` смешивал идемпотентность разового запроса с FR-37 (лимит обращений **за период**) — мешал легитимному повторному обращению через год | `UNIQUE` убран. Идемпотентность конкретного запроса — `Idempotency-Key`. Анти-спам лимит за период — Redis-счётчик в usecase (скользящее окно не выражается постоянным БД-constraint) |
| 39 | `moderation.queue_items` без UNIQUE на активную цель — повторная постановка того же `target` создаёт второй айтем, два модератора могут claim'ить одну цель через разные строки | Частичный `UNIQUE(target_type, target_id) WHERE status<>'resolved'`. Повторный флаг цели — `UPDATE` существующего pending-айтема, не `INSERT` |
| 40 | Идемпотентность консьюмера «по event_id» заявлена словами, без механизма — at-least-once Redis Streams гарантированно повторит доставку | `RatingSync.Apply` пишет абсолютные значения → естественно идемпотентен, dedup-стор не нужен, только явная формулировка. `communications`-уведомления НЕ идемпотентны по построению (повторный push реально уйдёт дважды) → `communications.processed_events(event_id PK)`, INSERT-based dedup |
| 41 | `analytics.profile_events` — партиционирование без джоба создания новых партиций (вставка упадёт на переходе месяца); `DROP PARTITION` в retention — несуществующий синтаксис Postgres | Джоб в `cmd/worker` создаёт партицию на месяц вперёд заранее, идемпотентно (E6.3.5). Retention исправлен на `DETACH PARTITION`+`DROP TABLE` |
| 42 | Перф-стратегия покрывала только веху 1 (`hey`+синтетические данные) — decay-пересчёт (веха 4), WS-хаб (веха 5), batch-writer (веха 6), миграции на заполненной БД нигде не измерялись/не тестировались | Точечные добавки в критерии готовности: веха 4 — перф-замер ночного пересчёта на 10-50к синтетических отзывов; веха 5 — soak-тест WS от 30 мин; веха 6 — `hey` с активным batch-writer'ом (конкретная команда, не только слова); задача 30 — CI-шаг `migrate-up→seed→migrate-down→migrate-up` |
| 43 | Критерий готовности вехи 3 требовал audit-запись на **любой** мутирующий эндпоинт — невыполнимо для обычных пользовательских действий (`audit_log` — журнал решений модерации, не общий лог); `actor_id NOT NULL` не покрывал системные действия (велосити-детектор, таймаут-джоб) | Критерий сужен до модераторских эндпоинтов конкретно. `actor_id` → nullable + `actor_type ∈ {user,system}` |
...
**Все находки go-reviewer от 2026-08-25 закрыты** (10 критичных + все важные, решения #10-43 в таблице выше). Второстепенные находки того же аудита (трассируемость FR в `.docx`, именование `area`/`district`, отсутствующие эндпоинты карточки сотрудника/новости-по-id, клиентское «избранное», потеря `foodMenu` при переезде, N/A для метрик transport/food, наименование `owner_type='staff'` vs фронтовый `type`, видимость контактов персонала, дублирование `views_count`, append-only enforcement на `audit_log`, тонкости `goose`+`CONCURRENTLY`, `PK` на `idempotency_keys`, профиль access-токена на клиенте/security-заголовки, владелец эмиссии analytics-событий, Google-вход не упомянут в Out of scope) — не рассматривались в этом проходе, минорны по классификации самого ревью.
