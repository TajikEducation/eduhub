...
**Остаётся отложенным на отдельный проход** (не критичные находки go-reviewer): ограниченная перф-стратегия (только веха 1), критерий готовности вехи 3 vs назначение `audit_log`.
